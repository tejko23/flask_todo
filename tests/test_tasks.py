        ...
        with app.app_context():
            ...
            for table in [
                ...
                if tasks_list:
                    ...
                    assert (
                        response.get_data(as_text=True).count("<tr>")
                        == len(tasks_list) + 1
                    )
                else:
...
def test_create(client, auth, app):
    ...
    response = client.post(
        "/create",
        data={"task": "created", "due-date": "2099-01-01"},
...
def test_update(client, auth, app):
    ...
    response = client.post(
        "/1/update",
        data={"task": "updated", "due-date": "2099-01-01"},
        content_type="multipart/form-data",
    )
...
def test_create_update_validate(client, auth, path):
    ...
    response = client.post(
        path, data={"task": "date", "due-date": "2020-01-01"}
    )
